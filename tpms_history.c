...
#define M_OPL_TPMSHistoryItemArray_t()                                         \
  ARRAY_OPLIST(TPMSHistoryItemArray, M_POD_OPLIST)
...
uint32_t tpms_history_get_frequency(TPMSHistory *instance, uint16_t idx) {
  furi_assert(instance);
  TPMSHistoryItem *item =
      TPMSHistoryItemArray_get(instance->history->data, idx);
  return item->preset->frequency;
}

SubGhzRadioPreset *tpms_history_get_radio_preset(TPMSHistory *instance,
                                                 uint16_t idx) {
  furi_assert(instance);
  TPMSHistoryItem *item =
      TPMSHistoryItemArray_get(instance->history->data, idx);
...
const char *tpms_history_get_protocol_name(TPMSHistory *instance,
                                           uint16_t idx) {
  furi_assert(instance);
  TPMSHistoryItem *item =
      TPMSHistoryItemArray_get(instance->history->data, idx);
  flipper_format_rewind(item->flipper_string);
  if (!flipper_format_read_string(item->flipper_string, "Protocol",
                                  instance->tmp_string)) {
...
bool tpms_history_get_text_space_left(TPMSHistory *instance,
                                      FuriString *output) {
  furi_assert(instance);
  if (instance->last_index_write == TPMS_HISTORY_MAX) {
    if (output != NULL)
      furi_string_printf(output, "Memory is FULL");
    return true;
  }
  if (output != NULL)
    furi_string_printf(output, "%02u/%02u", instance->last_index_write,
                       TPMS_HISTORY_MAX);
  return false;
}

void tpms_history_get_text_item_menu(TPMSHistory *instance, FuriString *output,
                                     uint16_t idx) {
  TPMSHistoryItem *item =
      TPMSHistoryItemArray_get(instance->history->data, idx);
...
TPMSHistoryStateAddKey tpms_history_add_to_history(TPMSHistory *instance,
                                                   void *context,
                                                   SubGhzRadioPreset *preset) {
  furi_assert(instance);
  furi_assert(context);
  ...
  if (instance->last_index_write >= TPMS_HISTORY_MAX)
    return TPMSHistoryStateAddKeyOverflow;
  ...
  for (size_t i = 0; i < TPMSHistoryItemArray_size(instance->history->data);
       i++) {
    TPMSHistoryItem *item =
        TPMSHistoryItemArray_get(instance->history->data, i);
    if (item->id == id) {
      sensor_found = true;
      Stream *flipper_string_stream =
          flipper_format_get_raw_stream(item->flipper_string);
      stream_clean(flipper_string_stream);
      subghz_protocol_decoder_base_serialize(decoder_base, item->flipper_string,
                                             preset);
  ...
  if (!sensor_found) { //-V547
    TPMSHistoryItem *item =
        TPMSHistoryItemArray_push_raw(instance->history->data);
